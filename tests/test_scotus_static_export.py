...
def test_complete_tree_has_archives_pagination_json_and_empty_state(tmp_path: Path) -> None:
    ...
    search = (output / "scotus/search/index.html").read_text(encoding="utf-8")
    assert "Search public case briefs" in home
    assert 'id="case-search"' in home
    assert 'id="search-results"' in home
    assert 'data-index="/ragchew/data/v1/search.json"' in home
    assert "Enter a search or choose a filter." in home
    assert "Automated legal analysis" in home
    assert "Search public case briefs" in search
    validate_static_candidate(output, urls)

    empty, _, _ = export(tmp_path, name="empty-bootstrap")
    assert not (empty / "scotus/cases").exists()
    assert "Search public case briefs" in (empty / "index.html").read_text(encoding="utf-8")
