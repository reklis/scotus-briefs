...
- Add a nightly and manually dispatchable GitHub Actions batch that discovers Court updates, processes only new or changed case material in ephemeral private workspace, merges unchanged validated public cases from the prior public snapshot, and atomically deploys a complete Pages artifact.
...
- Nightly processing still contacts only reviewed official Supreme Court endpoints and the configured official OpenAI API. Raw documents and extracted text exist only in ephemeral job storage and are never included in the Pages artifact or generated-content branch.
- Requires repository Pages configuration and protected GitHub environments/secrets. The project owner must confirm the final OSI software license and generated-content license before public launch; Apache-2.0 for repository-authored software/docs and CC BY 4.0 for original generated briefs are the proposed defaults, with official Court materials excluded and linked rather than redistributed.
