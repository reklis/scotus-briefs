...
- [ ] 1.3 Run bounded cold and warm strict-schema extraction and Citizen’s Guide probes; record only permitted identity, token, timing, expected-field, and resource observations and prove model cleanup. (Blocked: all exact `think=false` probes returned empty content; sanitized failure evidence and cleanup proof are recorded.)
- [x] 1.4 Conservatively project fixed-canary runtime under existing request, retry, token, model-call, cost, and five-hour limits. (Measured latency fits in isolation, but the all-empty protocol does not authorize a canary.)

## 2. Citizen’s Guide Prompt and Planning

- [x] 2.1 Add a versioned GPT-OSS Citizen’s Guide prompt profile that requests schema-only output, no visible reasoning, no more than 180 total words, and one or two ordinary-language sentences per applicable field.
...
- [x] 4.2 Bind prompt-profile version, schema, exact model identity, context ceiling, and generation controls into processor and request fingerprints and invalidate stale candidate, retry, and reviewer state.
...
- [ ] 5.1 Add prompt/schema tests for field counts, word and sentence bounds, plain language, immediate term explanation, role verbs, status-safe omission, and no visible reasoning or wrapper text.
- [x] 5.2 Add exact identity tests for success, missing artifact, digest drift, context drift, no `latest`, no native-context substitution, no hosted fallback, and fingerprint changes.
...
- [x] 5.4 Update configuration, security, architecture, model, licensing, and Pages operations documentation with the exact identity, Citizen’s Guide contract, validator policy, qualification procedure, and rollback.
